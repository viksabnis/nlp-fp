...
from delphin.mrs import EP, HCons, ICons, MRS
...
def itcleft_modern(orig_sem, ep):
    '''
    Modern ERG (2018+) version:
    Adds an ICONS "focus" constraint between the Event (ARG0) and the Argument.
    '''
    def itcleft_arg(orig_sem, ep, arg_id):
        # 1. Identify the variables
        event_var = ep.args.get('ARG0') # The verb event (e.g., e2)
        target_var = ep.args.get(arg_id) # The entity to focus (e.g., x5)

        if not event_var or not target_var:
            return None

        # 2. Create the ICONS element < event focus entity >
        # Note: The direction is important. Your log showed < e focus x >
        new_icon = ICons(left=event_var, relation='focus', right=target_var)

        # 3. Append to existing ICONS list
        current_icons = list(orig_sem.icons) if orig_sem.icons else []
        new_icons = current_icons + [new_icon]

        # 4. Return new MRS
        new_sem = copy_sem(orig_sem, icons=new_icons)
        return new_sem

    transforms = {}
    for i in get_extractable(ep):
        # Skip ARG1 for clefts (usually we cleft the object/subject-complement)
        # But if you want to cleft subject ("It is the dog that barks"), keep ARG1.
        res = itcleft_arg(orig_sem, ep, i)
        if res:
            transforms['it cleft: ' + i] = res
    return transforms


def passive_modern(orig_sem, ep, arg_id=None):
    '''
    Modern ERG (2018+) version:
    Attempts to force passive by setting the Object (ARG2) as the TOPIC via ICONS.
    '''
    def passive_arg(orig_sem, ep, arg_id):
        # 1. Get variables
        event_var = ep.args.get('ARG0')
        target_var = ep.args.get(arg_id) # This should be ARG2 (the object)

        if not event_var or not target_var:
            return None

        # 2. Create ICONS < entity topic event >
        # Making the object the topic often forces the generator to front it (Passive)
        new_icon = ICons(left=target_var, relation='topic', right=event_var)
    ...
    # Only ARG2 (Object) or ARG3 (Indirect Object) can usually be passivized
    return {'passive: ' + str(i): passive_arg(orig_sem, ep, i)
            for i in arg_ids if i != 'ARG1'}
...
def get_trans_list(orig_sem, qeqs, ep, rules=None, tenses=None, progs=None, perfs=None, modalities=None, use_modern=True):
    '''
    return list of possible transformations
    negation, tense, question, modal, it cleft, passive
    '''
    # 1. Fast Fail Checks
    if is_conjunction(ep) or is_unknown(ep) or is_question(orig_sem):
        return {}

    # 2. Defaults
    if not tenses: tenses = ['past', 'pres', 'fut']
    if not modalities: modalities = ['may']
    modalities = [f'_{x}_v_modal' for x in modalities]

    # 3. Dispatcher: Chooses Modern or Legacy based on flag
    rules_pool = {
        0: lambda x: negation(x, ep),
        1: lambda x: tense_aspect(x, ep.args['ARG0'], tenses=tenses, progs=progs, perfs=perfs),
        2: lambda x: modality(x, ep, modalities),
        4: lambda x: itcleft_modern(x, ep) if use_modern else itcleft(x, ep),
        5: lambda x: passive_modern(x, ep) if use_modern else passive(x, ep),
        6: lambda x: {'native': x},
    ...
    if not rules:
        ...
        # If specific rules were requested, filter the pool
        rules = {i: rules_pool[i] for i in rules if i in rules_pool}

    # 4. Special Handling for existing phenomena (avoid double-application)
    if is_itcleft(ep):
        rules = pop_all(rules, [0, 4])
        # Find the real verb inside the cleft to apply tense changes to that instead
        try:
            # Find the real verb inside the cleft (ARG2)
            inner_ep = find_eps(orig_sem, handle=ep.args['ARG2'])[0]
            # If we are applying tense changes (Rule 1), apply them to the inner verb
            if rules.get(1):
                rules[1] = lambda x: tense_aspect(
                    x, inner_ep.args['ARG0'], tenses=tenses, progs=progs, perfs=perfs)
            return get_trans_list(orig_sem, qeqs, inner_ep, rules=rules, use_modern=use_modern)
        except IndexError:
            return rules

    if is_modal(ep) or is_because_x(ep):
        try:
            arg_key = 'ARG1'
            inner_ep = find_eps(orig_sem, handle=ep.args[arg_key])[0]
            return get_trans_list(orig_sem, qeqs, inner_ep, rules=rules, use_modern=use_modern)
...
def transfer(orig_sent, grm, ace, timeout=5, rules=None, tenses=None, progs=None, perfs=None, modalities=None,
             print_orig_mrs=False, print_orig_tree=False, parsed_sem=None,
             parser=None, generator=None):

    # 0. Auto detect ERG version (1214 or not)
    use_modern = "1214" not in os.path.basename(grm)
    print(f"Zoy Debug: use_modern={use_modern}")

    # 1. Parse using persistent parser if available
    if not parsed_sem:
        try:
            orig = get_best_parse(orig_sent, grm, ace, timeout, parser_instance=parser)
            if not orig:
                return None
            orig_sem = orig.mrs()
    ...
    rules = get_trans_list(orig_sem, qeqs, index_ep, rules=rules, tenses=tenses,
                           progs=progs, perfs=perfs, modalities=modalities,
    ...
    for trans_type, s in simp_trans_sems.items():
        if trans_type == 'native': continue
        new_qeqs = get_qeqs(s)
        new_index_ep = get_index_ep(s)
        if new_index_ep is None: continue
        comp_rules = get_trans_list(s, new_qeqs, new_index_ep, rules=[4, 5], tenses=tenses,
                                    progs=progs, perfs=perfs, modalities=modalities,
                                    use_modern=use_modern)
    ...
    # 3. Generate using persistent generator if available
    for trans_type in simp_trans_sems:
        filter_fn = is_inverted if trans_type == 'inverted polar question' else lambda x: True

        type_transforms = generate(
            simp_trans_sems[trans_type], grm, ace, timeout,
            filter_func=filter_fn,
            generator_instance=generator # Pass the persistent generator
        )

        if type_transforms:
            type_transforms = [{'surface': x['surface']} for x in type_transforms]
            transforms.update({trans_type: type_transforms})

    # Always include original with MRS
    transforms['original'] = [{
        'surface': orig_sent,
...
def get_best_parse(sent, grm, ace, timeout, parser_instance=None):
    ''' Return highest ranked parse result. Uses persistent parser if provided. '''
    if parser_instance:
        # Use the open process
        results = parser_instance.interact(sent).results()
        if not results: return None
        return results[0]
    else:
        # Fallback: Open new process (Slow!)
...
def generate(sem, grm, ace, timeout, filter_func=lambda x: True, generator_instance=None):
    ''' Generate surface text from MRS. Uses persistent generator if provided. '''
    sem_enc = simplemrs.encode(sem, lnk=False, indent=True)
    if generator_instance:
        # Use the open process
        try:
            results = generator_instance.interact(sem_enc).results()
        except:
            return None
    else:
        # Fallback: Open new process (Slow!)
